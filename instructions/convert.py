...
class AssemblyConverter:
	...
	def write_to_file(output : list, file : str) -> None:
		...
		elif extension == '.txt' or extension == '.hex' or extension == '.mem':
	...
	def apply_hex(output : list) -> list:
		return ['{:08X}'.format(int(elem, 2)).lower() for elem in output]
...
if __name__ == "__main__":
	...
	convert = AssemblyConverter(output_mode = 'f', hex_mode = True)
	convert(args.input_file, args.output_file)

	# convert = AssemblyConverter(output_mode = 'p', hex_mode = True)
	# convert.convert("xcnv.lbkn 0(x5), x6")