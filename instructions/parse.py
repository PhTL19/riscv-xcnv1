...
class _Parser:
	...
	def handle_specific_instr(x : list) -> list:
		# for sw, lw, lb, lh, sb, sh, lbu, lhu
		if (len(x[0]) == 2 and (x[0] in S_instr or x[0] in I_instr)) or x[0] == 'lhu' or x[0] == 'lbu':
			y = x[-1].split('('); y[1] = y[1].replace(')','')
			return x[0:-1] + y
		elif x[0][0:6] == "xcnv.l" or x[0][0:6] == "xcnv.s" :
			y = x[1].split('('); y[1] = y[1].replace(')','')
			x.remove(x[1]); 
			x.insert(1, y[0])
			x.insert(2, y[1])
...
if __name__ == "__main__":
	print(Parser.tokenize("xcnv.lbkn 0(x5), x6"))